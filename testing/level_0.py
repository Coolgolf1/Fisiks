...
class FreehandDrawing:
    def __init__(self, space, points: list, thickness=5):
        ...
        for i in range(len(points) - 1):
            start = pymunk.Vec2d(points[i][0], points[i][1]) - \
                pymunk.Vec2d(self.body.position.x, self.body.position.y)
            end = pymunk.Vec2d(points[i + 1][0], points[i + 1][1]) - \
                pymunk.Vec2d(self.body.position.x, self.body.position.y)
...
class StaticLine:
    def __init__(self, space, points, thickness=10):
        ...
        self.line_shape = pymunk.Segment(
            self.body, points[0], points[1], thickness)
...
while running:
    ...
    for drawing in drawings:
        for shape in drawing.shapes:
            start_pos = (int(shape.body.position.x + shape.a.x),
                         int(shape.body.position.y + shape.a.y))
            end_pos = (int(shape.body.position.x + shape.b.x),
                       int(shape.body.position.y + shape.b.y))
